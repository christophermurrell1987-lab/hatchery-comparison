...
# ============================================
# TAB 3: HATCH CURVE (NEW!)
# ============================================
with tab3:
    st.subheader("🐣 Flock Hatch Curve")
    st.caption("Visualizing the full life cycle of a flock across all hatcheries (ignoring global date/hatchery filters).")
    
    # 1. Filter df_raw based on selected Line/Type (if any selected)
    if selected_types:
    ...
    if selected_flock_curve:
        ...
        ax_curve.set_title(f"Hatch Curve: Flock {selected_flock_curve}")
        ax_curve.set_xlabel("Flock Age (Weeks)")
        ax_curve.set_ylabel(f"{curve_metric} (%)")
        ax_curve.grid(True, linestyle='--', alpha=0.7)
        ax_curve.legend(title='Hatchery', bbox_to_anchor=(1.05, 1), loc='upper left')
        
        st.pyplot(fig_curve)
        
        # Optional: Show the data table below
        with st.expander("Show Raw Data for Curve"):
            display_cols = ['Egg Collect Date', 'Hatchery', 'Flock Age', 'Set', 'Hatch of Set']
            # Format date for display
            curve_data['Egg Collect Date'] = curve_data['Egg Collect Date'].dt.strftime('%d/%m/%y')
            st.dataframe(curve_data[display_cols].sort_values('Flock Age'), use_container_width=True, hide_index=True)

# ============================================
# TAB 4: COMPARISON CHARTS (Existing)
# ============================================
with tab4:
