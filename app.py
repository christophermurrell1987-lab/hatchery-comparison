...
# ============================================
# TAB 3: HATCH CURVE (FIXED HOVER DATA)
# ============================================
with tab3:
    ...
    if selected_flock_curve:
        ...
        # Ensure Egg Age is populated. If missing, look for 'Egg age at Set'
        if 'Egg Age' not in curve_data.columns and 'Egg age at Set' in curve_data.columns:
             ...
             # Fill any empty egg ages with Egg age at Set if available, or just NaN
        ...
        # Rename columns to make Hover Label cleaner automatically
        # This is safer than using %{customdata[i]} which causes the index mismatch bug
        ...
        # Ensure Date Format string exists for tooltip
        ...
        fig = px.line(
            ...
            # Use columns directly here. Plotly Express handles the mapping.
            hover_data={
                "Flock Age": True,
                "Hatchery": True,
                "Date Str": True,
                "Egg Age (Days)": ":.1f", # Format float
                "Eggs Set": True,
                curve_metric: ":.2f"      # Format metric
            },
            labels={"Date Str": "Date"} # Rename label in tooltip
        )
        ...
        with st.expander("Show Raw Data for Curve"):
            # Prepare table for display (using original names/renamed names)
            display_cols = ['Collect Date', 'Hatchery', 'Flock Age', 'Egg Age (Days)', 'Eggs Set', curve_metric]
            
            # Format Date column for Table (it is currently datetime)
