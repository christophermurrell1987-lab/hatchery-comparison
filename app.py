...
# Calculate min/max dates from the CLEANED dataset
if not df.empty:
...
selected_hatcheries = st.sidebar.multiselect(
    ...
    help="Select multiple hatcheries. The tool will show overlapping batches for ANY combination of these selections."
)
...
# LOGIC CONFIRMATION: This ensures we get ANY overlaps (A vs B), not just (A vs B vs C)
...
with tab1:
    ...
    st.caption("Shows performance differences for any hatcheries that shared batches within your selection.")
    
    if not comparison.empty:
        hatcheries = sorted(comparison['Hatchery'].unique())
        pairwise_results = []
        
        # Loop through EVERY combination of selected hatcheries
        ...
        if pairwise_results:
            ...
            st.warning("No overlapping batches found between the selected hatcheries.")
    else:
        st.info("Select at least two hatcheries with overlapping batches to see comparisons.")

# ============================================
# TAB 2: INDIVIDUAL FLOCKS (Merged Variance)
# ============================================
with tab2:
    ...
    if not comparison.empty:
        ...
        if not df_display.empty:
            ...
            # Ensure Date object for config
            pivot_display['Egg Collect Date'] = pd.to_datetime(pivot_display['Egg Collect Date']).dt.date
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Egg Collect Date": st.column_config.DateColumn(
                        "Collection Date",
                        format="DD/MM/YY" # UK Format Display
                    )
                }
            )
        else:
...
if not comparison.empty:
    ...
    # Export Dates as UK Format Strings
    comparison_export['Egg Collect Date'] = comparison_export['Egg Collect Date'].apply(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if 'pivot_display' in locals():
            ...
            # Format pivot dates for export
            pivot_export['Egg Collect Date'] = pivot_export['Egg Collect Date'].apply(
                lambda x: x.strftime('%d/%m/%y') if pd.notnull(x) else ''
            )
