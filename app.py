...
def load_hatchery_data_v5():
    ...
    # ----------------------------------------
    # DATA CLEANING - TYPE / LINE (Fixed Splitting Issue)
    # ----------------------------------------
    if 'Type' in df.columns:
        # 1. Force string format
        df['Type'] = df['Type'].astype(str)
        
        # 2. Force UPPERCASE (Fixes 'Phns' vs 'PHns')
        df['Type'] = df['Type'].str.upper()
        
        # 3. Strip leading/trailing whitespace
        df['Type'] = df['Type'].str.strip()
        
        # 4. Remove internal spaces (Fixes '41 PED' vs '41PED')
        df['Type'] = df['Type'].str.replace(' ', '', regex=False)
        
        # 5. Standardize suffixes (e.g. PHNS -> PH)
