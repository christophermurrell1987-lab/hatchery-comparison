...
with tab1:
    ...
    for i, (hatchery, row) in enumerate(hatchery_summary.iterrows()):
        with cols[i]:
            st.metric(
                label=hatchery,
                value=f"{row['Avg HoS %']:.2f}%",  # CHANGED TO .2f
        ...
        for h2 in hatcheries[i+1:]:
            ...
            if len(shared) > 0:
                ...
                pairwise_results.append({
                    ...
                    'H1 Avg HoS %': round(h1_data['Hatch of Set'].mean() * 100, 2), # CHANGED TO 2 DP
                    'H2 Avg HoS %': round(h2_data['Hatch of Set'].mean() * 100, 2), # CHANGED TO 2 DP
                    'Difference %': round(diff, 2), # CHANGED TO 2 DP
                    'H1 Avg HoF %': round(h1_data['Hatch Of Fertile'].mean() * 100, 2), # CHANGED TO 2 DP
                    'H2 Avg HoF %': round(h2_data['Hatch Of Fertile'].mean() * 100, 2)  # CHANGED TO 2 DP
                })
...
with tab4:
    ...
    pivot = pivot.round(2) # CHANGED TO 2 DP
...
with tab5:
    ...
    sns.heatmap(heatmap_data, annot=True, fmt='.2f', cmap='RdYlGn', ax=ax3) # CHANGED TO .2f
...
for col in ['Hatch of Set', 'Hatch Of Fertile', 'Fertility %', 'Act Dead %', 'Banger %']:
    comparison_export[col] = (comparison_export[col] * 100).round(2) # CHANGED TO 2 DP
