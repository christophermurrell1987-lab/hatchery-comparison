...
with tab2:
    ...
    st.dataframe(
        high_var[display_cols].style.format({
            'Avg Egg Age': '{:.0f}',
            'Avg HoS %': '{:.2f}',
            'Min HoS %': '{:.2f}',
            'Max HoS %': '{:.2f}',
...
with tab3:
    ...
    # pivot_display has columns like: ('Egg Collect Date',''), ('Hatchery','Metric'), ('Hatchery','Egg age at Set')
...
with pd.ExcelWriter(output, engine='openpyxl') as writer:
    ...
    # Apply rounding to values for export since styling doesn't affect values
