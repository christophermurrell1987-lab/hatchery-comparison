...
# ============================================
@st.cache_data
def load_hatchery_data_v3():
    """Load and cache the default hatchery data with strict date cleaning"""
    ...
    if 'Egg Collect Date' in df.columns:
        ...
        # 2. Filter out 1900 errors immediately
        df = df[df['Egg Collect Date'].dt.year >= 2000]
        
        # 3. CRITICAL: Remove time component entirely from the object
        # This ensures '2025-01-01 09:00' == '2025-01-01 00:00' for matching
...
# BATCH MATCHING
# ============================================
# Create Batch Key using STRIPPED date string to guarantee matches ignore time
df['Batch_Key'] = df['Egg Collect Date'].dt.strftime('%Y-%m-%d') + '_' + df['flock_number'].astype(str)
...
# "Inclusive Overlap": If a batch is in >1 of the selected hatcheries, keep it.
...
with tab2:
    ...
    if not comparison.empty:
        ...
        if not df_display.empty:
            ...
            # Ensure proper datetime object for display
            pivot_display['Egg Collect Date'] = pd.to_datetime(pivot_display['Egg Collect Date'])
...
if not comparison.empty:
    ...
    # Clean export date format (dd/mm/yy)
