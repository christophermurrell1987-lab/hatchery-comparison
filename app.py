...
import plotly.express as px  # NEW: Import Plotly for interactive charts
...
def load_hatchery_data_v9():
    ...
    if 'Flock Age' in df.columns:
        # Force numeric and round to integer to align data points
...
# Apply Grouping Logic to main df
...
# ============================================
# COMPARISON LOGIC (BATCH MATCHING)
# ============================================
    ...
    if not comparison.empty:
        ...
        if not df_display.empty:
            ...
            st.dataframe(
                ...
                column_config={"Comparison_Date": st.column_config.DateColumn("Date", format="DD/MM/YY")}
            )
        else:
...
# ============================================
# TAB 3: HATCH CURVE (INTERACTIVE PLOTLY)
# ============================================
with tab3:
    st.subheader("🐣 Flock Hatch Curve")
    st.caption("Visualizing the full life cycle of a flock across all hatcheries.")
    
    # 1. Filter df_raw based on selected Line/Type
    if selected_types:
    ...
    if selected_flock_curve:
        ...
        if 'Egg Age' not in curve_data.columns:
             curve_data['Egg Age'] = np.nan
        
        # Create formatted strings for hover (easier to read)
        curve_data['Formatted Date'] = curve_data['Egg Collect Date'].dt.strftime('%d/%m/%y')
        curve_data['Metric Value'] = curve_data[curve_metric].round(2)
        curve_data['Egg Age (Days)'] = curve_data['Egg Age'].round(1)
        
        # Sort by Age for correct line drawing
        ...
        fig = px.line(
            ...
            hover_data={
                "Flock Age": True,
                curve_metric: False, # Hidden because we mapped 'Metric Value'
                "Hatchery": True,
                "Formatted Date": True,
                "Egg Age (Days)": True,
                "Metric Value": True,
            ...
            labels={
                "Flock Age": "Flock Age (Weeks)",
                curve_metric: f"{curve_metric} (%)",
                "Formatted Date": "Collect Date"
            }
        )
        
        # Improve Tooltip Layout
        fig.update_traces(
            hovertemplate="<b>%{y:.2f}%</b><br>Flock Age: %{x} wks<br>Egg Age: %{customdata[3]} days<br>Date: %{customdata[2]}<br>Set: %{customdata[5]}"
        )
        
        # Customize Layout
        fig.update_layout(
            hovermode="x unified", # Shows all lines' data for that week
            legend=dict(
                orientation="h",
                yanchor="bottom",
                y=1.02,
        ...
        with st.expander("Show Raw Data for Curve"):
            display_cols = ['Egg Collect Date', 'Hatchery', 'Flock Age', 'Egg Age', 'Set', 'Hatch of Set']
            curve_data_table = curve_data.copy()
            curve_data_table['Egg Collect Date'] = curve_data_table['Egg Collect Date'].dt.strftime('%d/%m/%y')
            curve_data_table['Hatch of Set'] = curve_data_table['Hatch of Set'].round(2)
