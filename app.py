...
with tab4:
    ...
    # Reorder columns to group by Hatchery: (Hatchery A -> Metric, Egg Age), (Hatchery B -> Metric, Egg Age)
    # Current columns are MultiIndex: Level 0 = [Metric, Egg Age], Level 1 = [Hatchery A, Hatchery B]
    # We swap to: Level 0 = [Hatcheries], Level 1 = [Metric, Egg Age]
    pivot = pivot.swaplevel(0, 1, axis=1).sort_index(axis=1)
    
    # Reset index to clean up rows for display
    ...
    st.dataframe(pivot_display, use_container_width=True, height=600)
...
with pd.ExcelWriter(output, engine='openpyxl') as writer:
    ...
    pivot_display.to_excel(writer, sheet_name='Pivot_Table')
