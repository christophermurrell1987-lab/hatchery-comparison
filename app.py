...
# Line / Type Filter (NEW)
st.sidebar.subheader("Select Line / Type")
all_types = sorted(df['Type'].astype(str).unique().tolist()) if not df.empty and 'Type' in df.columns else []
...
if not df.empty:
    ...
    # Filter Line/Type
    if 'Type' in df.columns:
        df = df[df['Type'].astype(str).isin(selected_types)]
...
# BATCH MATCHING
# ============================================
# Create Batch Key using Comparison_Date (Daily or Weekly)
...
# Note: We group by Comparison_Date now, not Egg Collect Date
...
if not comparison.empty:
    ...
    # Use Comparison_Date for export (shows either daily date or weekly start date)
