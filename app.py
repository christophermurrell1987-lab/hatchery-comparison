...
# ============================================
# SIDEBAR - DATA CLEANING (MOVED TO TOP)
# ============================================
...
# Calculate min/max dates, EXCLUDING year 1900 errors
if not df.empty:
    # Filter out dates before year 2000 (removes 1900-01-01 errors)
    valid_dates = df[df['Egg Collect Date'].dt.year > 2000]['Egg Collect Date']
    
    if not valid_dates.empty:
        min_date = valid_dates.min().date()
        max_date = valid_dates.max().date()
    else:
        # Fallback if all dates are weird
...
start_date = st.sidebar.date_input(
    ...
    format="DD/MM/YYYY"  # UK Format Input
)
...
# Tabs for different views - REDUCED TO 3 TABS
...
with tab2:
    ...
    if not comparison.empty:
        # 1. Filter by Variance first if threshold > 0
        ...
        if not df_display.empty:
            ...
            # Ensure Egg Collect Date is datetime/date object for sorting and formatting
            ...
            # Use column_config to display date in UK format "DD/MM/YY"
            st.dataframe(
                pivot_display.style
...
if not comparison.empty:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if 'pivot_display' in locals():
            ...
            # Format pivot dates for export too
