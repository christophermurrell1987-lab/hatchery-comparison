...
# Hatchery filter
st.sidebar.subheader("Focus Hatcheries")
all_hatcheries = sorted(df['Hatchery'].unique().tolist()) if not df.empty else []
selected_hatcheries = st.sidebar.multiselect(
    "Filter by batches present in:",
    all_hatcheries,
    default=all_hatcheries,
    help="Select a hatchery to see all batches it processed, including data from OTHER hatcheries that shared those same batches."
)
...
# ============================================
if not df.empty:
    # 1. First, create a working copy of the dataframe with the Date Filter applied
    df_date_filtered = df[(df['Egg Collect Date'].dt.date >= start_date) & 
                          (df['Egg Collect Date'].dt.date <= end_date)]

    # 2. Create Batch Key
    df_date_filtered['Batch_Key'] = df_date_filtered['Egg Collect Date'].astype(str) + '_' + df_date_filtered['flock_number'].astype(str)

    # 3. Identify relevant batches based on SELECTED hatcheries
    #    "Show me batches that exist in Elsing Lane..."
    relevant_batches = df_date_filtered[df_date_filtered['Hatchery'].isin(selected_hatcheries)]['Batch_Key'].unique()

    # 4. Now retrieve ALL records for those relevant batches from the date-filtered set
    #    "...but show me the data for these batches from ALL hatcheries (e.g. Spain, Germany)"
    df = df_date_filtered[df_date_filtered['Batch_Key'].isin(relevant_batches)]
    
    # Check if we have data left
    if df.empty:
        st.warning("No batches found matching criteria.")
        st.stop()
else:
